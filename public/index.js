...
formSignin.onsubmit = async e => {
  e.preventDefault();
  const signinDetails = await signin({ name: formSignin.nombre.value, 
                                        email: formSignin.email.value, 
                                        password: formSignin.password.value });
  console.log(signinDetails);
  if (signinDetails.error) {
    pStatus.innerHTML = signinDetails.error;
    console.log(signinDetails.error);
    if (signinDetails.error.includes("users_user_email_key")) {
      pStatus.innerHTML = "correo electronico ya usado";
    }
    return;
  }

  pStatus.innerHTML = `Usuario creado satisfactoriamente! </br> Hola ${formSignin.nombre.value}</br> `;
}
